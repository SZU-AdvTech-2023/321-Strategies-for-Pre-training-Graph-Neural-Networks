...
def main():
    ...
    parser.add_argument('--input_model_file', type=str, default='', # ./myModel/Masking50
                        help='filename to read the model (if there is any)') #  new add

    parser.add_argument('--output_model_file', type=str, default = './myModel/Contextpred50', help='filename to output the model')
    ...
    if not args.input_model_file == "":
        model_substruct.load_state_dict(torch.load(args.input_model_file + ".pth", map_location=lambda storage, loc: storage)) # new add

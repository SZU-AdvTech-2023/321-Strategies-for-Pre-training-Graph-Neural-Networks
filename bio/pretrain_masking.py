...
def main():
    ...
    parser.add_argument('--input_model_file', type=str, default='./MyModel/Contextpred50',
                        help='filename to read the model (if there is any)') #  new add

    parser.add_argument('--model_file', type=str, default = './MyModel/Contextpred50Masking', help='filename to output the model')
    ...
    #set up dataset
    root_unsupervised = 'bio_dataset/dataset/unsupervised'
    ...
    if not args.input_model_file == "":
        model.load_state_dict(torch.load(args.input_model_file + ".pth", map_location=lambda storage, loc: storage)) # new add

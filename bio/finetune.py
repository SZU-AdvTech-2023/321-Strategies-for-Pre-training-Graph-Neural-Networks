...
def main():
    ...
    parser.add_argument('--model_file', type=str, default = './MyModel/pretrainMaskSupervised.pth', help='filename to read the model (if there is any)')
    parser.add_argument('--filename', type=str, default = 'finetune.txt', help='output filename')
    ...
    root_supervised = 'bio_dataset/dataset/supervised'
    print(66666)
    ...
    '''Python有很多留存数据的方式，其中一种是使用pickle模块。pickle模块可以将Python对象序列化为字节流进行存储，
    同时可以在需要时将其反序列化，并还原为原始对象。pickle.dump(obj, file, protocol=None, *, fix_imports=True)是其中一个方法，
    用于将Python对象obj序列化并存储到文件file中。
    例如：
